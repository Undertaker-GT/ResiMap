...
function setupResponsive() {
  ...
  // Ajustar controles según dispositivo
  if (window.innerWidth <= 768) {
    // Móviles: ocultar algunos controles o ajustar posición
...
const sectores = {
    ...
    "11": {
    name: "Sector 11",
    coords: [14.411585471566061, -90.68641365013927],
    area: [
    ...
    "16": {
    name: "Sector 16",
    coords: [14.411971482649188, -90.68625212640295],
    area: [
    ...
    casas: {
      "1": [14.413206589248427, -90.68259321975229],
      "2": [14.41329686238454, -90.68257645594697],
      "3": [14.41337544544749, -90.68257980871086],
      "4": [14.413455976819934, -90.68260394859075],
      "5": [14.41353585871749, -90.68265960442983],
      "6": [14.413561187118663, -90.68274543512281],
      "7": [14.413584567178004, -90.68282858359817],
      "8": [14.413620936155388, -90.68288528005579],
      "9": [14.41364802957008, -90.68296391875815],
      "10": [14.413680501861519, -90.68303700895414],
      "11": [14.413711675256629, -90.68311076969908],
      "12": [14.413740250864825, -90.68318922430801],
      "13": [14.413956516148188, -90.68310071140726],
      "14": [14.413914302219434, -90.68303432672943],
      "15": [14.413887674968239, -90.68296458929407],
      "16": [14.413827276558605, -90.68281438559104],
      "17": [14.413794804288626, -90.6827513536811],
      "18": [14.413775970370056, -90.68268765122087],
      "19": [14.413746745320406, -90.68261456102512],
      "20": [14.413712324700809, -90.68255219966194],
      "21": [14.413709081870305, -90.68255487051871],
      "22": [14.413610700097838, -90.68239156316827],
      "23": [14.413534714903031, -90.6823600472084],
      "24": [14.413465224147014, -90.68234931837192],
      "25": [14.41339119135122, -90.68234731618794],
      "26": [14.41328663041914, -90.6823520100527],
      "27": [14.413210645116513, -90.68235938612958],
      "28": [14.413124918077267, -90.68235737447293],
      "29": [14.413061272223787, -90.68236475054728]
    },
    area: [
      [14.414003, -90.683116], 
      [14.413730, -90.682416], 
    ...
    "44": {
    name: "Sector 44",
    coords: [14.411592965826513, -90.6868765919525],
    casas: {
    ...
    "45": {
    name: "Sector 45",
    coords: [14.411696116361263, -90.68663076953824],
    casas: {
    ...
    "46": {
    name: "Sector 46",
    coords: [14.412085338391202, -90.68645507354917],
    casas: {
...
// Función para manejar el clic en el botón de ruta desde el popup
document.addEventListener('click', function(e) {
  if (e.target && e.target.classList.contains('route-btn')) {
    const sectorId = e.target.getAttribute('data-sector');
    trazarRutaASector(sectorId); // ✅ RUTA DIRECTA
  }
});

// Función para buscar sector y mostrar ruta
...
// Reemplaza esta sección al final del archivo:
// Agrega después de crear el mapa
L.control.locate({
  position: 'topleft',
  drawCircle: true,
  flyTo: true, // Solo vuela a la ubicación cuando el usuario lo pide
...
const locateControl = L.control.locate({
  ...
  strings: {
    ...
    popup: "Estás aquí",
    outsideMapBoundsMsg: "Estás fuera del área del mapa"
  },
  clickBehavior: {
    inView: 'setView',
    outOfView: 'setView',
...
// Inicializar el modal de búsqueda
function initSearchModal() {
  ...
  sortedSectorIds.forEach(sectorId => {
    ...
    // Opción para el select (compatibilidad)
...
// Filtrar sectores según la búsqueda - VERSIÓN FINAL
...
function trazarRutaASector(sectorKey) {
  ...
  // 🔹 Si el usuario ingresó casa y existe
...
// Mantener la función buscarSector original para compatibilidad
function buscarSector() {
  const sectorId = document.getElementById("sectorSelect").value;
  if (!sectorId) return;

  toggleSearchModal(); // 🔍 SOLO MODAL
  trazarRutaASector(sectorId);
}
